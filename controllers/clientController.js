...
const { validateClient } = require("../utils/clientValidations");
...
const updateClientInfo = async (req, res) => {
  ...
  const clientFound = await clientInfo.findById(req.params._id);
  ...
  //   } = req.body;
  if (clientFound) {
    clientFound.name = req.body.name ? req.body.name : clientFound.name;
    clientFound.email = req.body.email ? req.body.email : clientFound.email;
    clientFound.dob = req.body.dob ? req.body.dob : clientFound.dob;
    clientFound.age = req.body.age ? req.body.age : clientFound.age;
    clientFound.phone = req.body.phone ? req.body.phone : clientFound.phone;
    clientFound.destination = req.body.destination ? req.body.destination : clientFound.destination;
...
const deleteClient = async (req, res) => {
  const foundClient = await clientInfo.findById(req.params._id);
  if (foundClient) {
    foundClient.remove();
    res.json({
